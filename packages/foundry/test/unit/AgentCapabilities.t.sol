...
contract AgentCapabilitiesTest is Test {
    ...
    function setUp() public {
        ...
        // Mint a test pet
        vm.prank(hook);
        petId = petRegistry.hatchFromHook(owner, 0, CHAIN_ID, poolId, positionId, -60, 60);
    }
    ...
    function testAgentCanTrackMultiplePets() public {
        ...
        vm.startPrank(hook);
        uint256 pet2 = petRegistry.hatchFromHook(owner2, 0, CHAIN_ID, pool2, 67890, -60, 60);
        uint256 pet3 = petRegistry.hatchFromHook(owner3, 0, CHAIN_ID, pool3, 11111, -60, 60);
        vm.stopPrank();
    ...
    function testAgentCanBatchUpdateHealth() public {
        ...
        for (uint256 i = 0; i < 5; i++) {
            ...
            vm.prank(hook);
            tokens[i] = petRegistry.hatchFromHook(owners[i], 0, CHAIN_ID, pool, uint256(i + 100), -60, 60);
        }
    ...
    function testBatchHealthUpdateGasUsage() public {
        ...
        for (uint256 i = 0; i < 10; i++) {
            ...
            vm.prank(hook);
            tokens[i] = petRegistry.hatchFromHook(petOwner, 0, CHAIN_ID, keccak256(abi.encodePacked("pool", i)), i + 200, -60, 60);
        }
    ...
    function testAgentTracksMultipleOwnerPets() public {
        ...
        vm.startPrank(hook);
        uint256 pet2 = petRegistry.hatchFromHook(owner2, 0, CHAIN_ID, keccak256("pool2"), 222, -60, 60);
        uint256 pet3 = petRegistry.hatchFromHook(owner3, 0, CHAIN_ID, keccak256("pool3"), 333, -60, 60);
        vm.stopPrank();
